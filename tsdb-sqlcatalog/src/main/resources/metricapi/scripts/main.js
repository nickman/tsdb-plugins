...
WebSocketAPIClient.prototype.sendRequest = function _sendRequest_(payload, routing) {
	try {
		if(routing==null) throw new Error("No routing");
		if(routing.callbacks==null) throw new Error("No routing response callbacks. (What's the point ?)");
		var cbcnt = 0;
		var cbs = ['onstart', 'onend', 'data', 'error'];
		cbs.forEach(function(cb){
			if(typeof routing.callbacks[cb] == 'function') cbcnt++;
		});
		if(cbcnt==0) throw new Error("Must provide at least one of the following callbacks [" + cbs.join(", ") + "]" );
	} catch (e) {
		throw new Error("Failed to validate routing:" + (e.message || e));
	}
	var RID = this.ridseq++;
	if(routing.respsel==null) routing.respsel = WebSocketAPIClient.filters.reridFilter(RID);
	if(routing.timeout==null) routing.timeout = 1500;
	if(routing.resetting==null) routing.resetting = false;
	var timeout = routing.timeout;	
	this.bus.push({
		key     : 'out/' + payload.svc + "/" + payload.op,     // this is just for routing stats
		payload : payload,
		routing : routing,
	});
};
