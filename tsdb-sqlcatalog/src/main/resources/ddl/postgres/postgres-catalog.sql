...
CREATE VIEW TSD_ROWCOUNTS AS 
select 'TSMETA'as TSMETA, count(*) as ROWS from tsd_tsmeta
UNION
select 'TAGV'as TAGV, count(*) as ROWS from tsd_tagv
UNION
select 'TAGK'as TAGK, count(*) as ROWS from tsd_tagk
UNION
select 'METRIC'as METRIC, count(*) as ROWS from tsd_metric
UNION
select 'ANNOTATION'as ANNOTATION, count(*) as ROWS from tsd_annotation
UNION
select 'TSD_LASTSYNC_FAILS' as TSD_LASTSYNC_FAILS, count(*) as ROWS from tsd_lastsync_fails
UNION
select 'TSD_LASTSYNC' as TSD_LASTSYNC, count(*) as ROWS from tsd_lastsync
UNION
select 'TSD_TAGPAIR' as TSD_TAGPAIR, count(*) as ROWS from tsd_tagpair
UNION
select 'TSD_FQN_TAGPAIR' as TSD_FQN_TAGPAIR, count(*) as ROWS from tsd_fqn_tagpair
ORDER BY 2 DESC;
